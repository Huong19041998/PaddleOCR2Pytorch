...
class RecConverter(BaseOCRV20):
    def __init__(self, config,src_model_path, **kwargs):
        ...
        self.version_model = os.path.basename(src_model_path).split("_")[1][-1]
    ...
    def del_invalid_state_dict(self, para_state_dict):
        ...
        for i, (k,v) in enumerate(para_state_dict.items()):
            ...
            if k.startswith('head.before_gtc'):
                continue
    ...
    def load_paddle_weights(self, paddle_weights):
        ...
        for k,v in para_state_dict.items():
            ...
            if self.version_model == '3':
                ptname = ptname.replace( 'head.ctc_encoder.encoder.','neck.encoder.')
                ptname = ptname.replace('head.ctc_head.fc.','head.fc.')
            try:
...
if __name__ == '__main__':
    ...
    converter = RecConverter(cfg,args.src_model_path)
    np.random.seed(666)
    ...
    save_name =  '{}_ptocr_v{}_rec_infer.pth'.format(name[:2],name.split("_")[1][-1])
